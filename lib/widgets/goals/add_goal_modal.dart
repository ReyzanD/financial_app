...
class _AddGoalModalState extends State<AddGoalModal> {
  ...
  void initState() {
    ...
    if (initial != null) {
      ...
      num? targetValue = initial['target_amount_232143'] as num?;
      ...
      final type =
          initial['goal_type_232143']?.toString() ??
          initial['goal_type']?.toString() ??
      ...
      final deadlineStr =
          initial['target_date_232143']?.toString() ??
          initial['target_date']?.toString() ??
          initial['deadline']?.toString();
      ...
      final priorityValue = initial['priority_232143'] ?? initial['priority'];
  ...
  Future<void> _submitGoal() async {
    ...
    try {
      ...
      if (_isEdit) {
        ...
        await _goalService.updateGoal(goalId, goalData);
        LoggerService.success('Goal updated successfully');

        if (mounted) {
          ErrorHandlerService.showSuccessSnackbar(
            context,
            l10n?.target_updated_successfully ?? 'Target berhasil diperbarui!',
          );
        ...
        await _goalService.addGoal(createData);
        ...
        if (mounted) {
          ErrorHandlerService.showSuccessSnackbar(
            context,
            l10n?.target_added_successfully ?? 'Target berhasil ditambahkan!',
          );
          Navigator.pop(context, true);
        }
      }
