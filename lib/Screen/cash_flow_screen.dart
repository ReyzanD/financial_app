...
class _CashFlowScreenState extends State<CashFlowScreen> {
  ...
  Future<void> _loadData() async {
    try {
      ...
      final results = await Future.wait([
        _cashFlowService.getCashFlowSummary(),
        _cashFlowService.getWeeklyCashFlowForecast(weeks: 4),
        _cashFlowService.forecastDailyCashFlow(days: 14),
      ]);
      ...
      setState(() {
        ...
        _weeklyForecast = results[1] as Map<String, dynamic>;
        _dailyForecast = results[2] as List<Map<String, dynamic>>;
        _isLoading = false;
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Widget _buildBody(BuildContext context, AppLocalizations? l10n) {
    ...
    return ListView(
      padding: const EdgeInsets.all(16),
      children: [
        _buildWeeklyForecast(context, l10n),
        const SizedBox(height: 16),
        _buildDailyForecast(context, l10n),
        const SizedBox(height: 16),
        _buildRiskWeeks(context, l10n),
      ],
    );
  }
  ...
  Widget _buildDailyForecast(BuildContext context, AppLocalizations? l10n) {
  ...
  Widget _buildRiskWeeks(BuildContext context, AppLocalizations? l10n) {
    final riskWeeks = (_weeklyForecast['risk_weeks'] as num?)?.toInt() ?? 0;
