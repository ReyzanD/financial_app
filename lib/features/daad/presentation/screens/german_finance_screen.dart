...
class _GermanFinanceScreenState extends State<GermanFinanceScreen> {
  ...
  int get _monthsToGoal =>
      _monthlySavings > 0
          ? (_sperrRequiredEur * (_rateEurToIdr ?? 17500) / _monthlySavings)
              .ceil()
          : -1;
  ...
  Future<void> _createDaadGoal() async {
    ...
    final goal = await showModalBottomSheet<Map<String, dynamic>>(
      ...
      builder: (_) => AddGoalModal(
        initialGoal: {
          'name': 'Sperrkonto Studi Jerman',
          'target_amount': idrTarget,
          'monthly_target': monthlyTarget,
          'target_date': DateTime.now().add(const Duration(days: 365))
              .toIso8601String(),
          'type': 'education',
        },
      ),
            ...
            children: [
              Icon(
                _monthlySavings >= _requiredInIdr
                    ...
                    ? Iconsax.clock
                    : Iconsax.info_circle,
                color: _monthlySavings >= _requiredInIdr
