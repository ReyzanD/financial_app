...
class AIBudgetController extends ChangeNotifier {
  ...
  void setEditedIncome(double? income) {
    _editedIncome = income;
    notifyListeners();
  }

  void setEditedPercentage(String category, double pct) {
    _editedPercentages[category] = pct;
    notifyListeners();
  }
  ...
  Map<String, dynamic>? _findMatchingCategory(List categories, String name) {
    ...
    for (var c in categories) {
      ...
      for (var p in possible) {
        if (cn.contains(p.toLowerCase())) return c;
      }
    }
    return null;
  }
