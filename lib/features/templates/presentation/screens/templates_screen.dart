...
class _TemplatesScreenState extends State<TemplatesScreen> {
            ...
            child: Column(
              ...
              children: [
                ...
                if (categoryName.isNotEmpty)
                  Text(categoryName, style: GoogleFonts.poppins(color: DesignTokens.textSecondaryDark, fontSize: 12)),
                Text(
      ...
      child: Column(
        ...
        children: [
          ...
          ElevatedButton(
            ...
            child: Text(l10n?.retry ?? 'Retry', style: GoogleFonts.poppins(color: Colors.white)),
          ),
        ],
      ),
    );
  ...
  Future<void> _quickAdd(TransactionTemplateModel template, TemplateController ctrl) async {
    try {
      await getIt<TransactionTemplateDataService>().createTransactionFromTemplate(template.id);
      if (!mounted) return;
      final l10n = AppLocalizations.of(context);
      final name = template.name;
      ErrorHandlerService.showSuccessSnackbar(context, '$name ${l10n?.added ?? 'added'}');
      await ctrl.loadData();
    } catch (e) {
  ...
  Future<void> _deleteTemplate(String templateId, TemplateController ctrl) async {
    final l10n = AppLocalizations.of(context);
    final confirmed = await showDialog<bool>(
      ...
      builder:
          (c) => AlertDialog(
            ...
            actions: [
              TextButton(onPressed: () => Navigator.pop(c, false), child: Text(l10n?.cancel ?? 'Cancel')),
              TextButton(
                onPressed: () => Navigator.pop(c, true),
                child: Text(l10n?.delete ?? 'Delete', style: const TextStyle(color: DesignTokens.errorColor)),
              ),
            ],
          ),
    );
    if (confirmed == true) {
      try {
        await getIt<TransactionTemplateDataService>().deleteTransactionTemplate(templateId);
        if (mounted)
...
class _AddTemplateModalState extends State<_AddTemplateModal> {
  final TransactionTemplateDataService _templateData = getIt<TransactionTemplateDataService>();
          ...
          child: Column(
            ...
            children: [
              ...
              TextFormField(
                ...
                validator: (v) => (v == null || v.isEmpty) ? 'Amount cannot be empty' : null,
              ),
              const SizedBox(height: DesignTokens.spacing6),
              SizedBox(
