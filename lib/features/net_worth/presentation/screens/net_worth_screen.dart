...
class _NetWorthScreenState extends State<NetWorthScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: Consumer<NetWorthController>(
        builder: (context, ctrl, _) => FloatingActionButton(
  ...
  Future<void> _recordSnapshot(BuildContext context) async {
    try {
      ...
      if (!mounted) return;
      ErrorHandlerService.showSuccessSnackbar(context, 'Snapshot recorded');
    } catch (e) {
      if (!mounted) return;
      ErrorHandlerService.showErrorSnackbar(
        context, ErrorHandlerService.getUserFriendlyMessage(e),
      );
    }
  }
            ...
            children: [
              ...
              Text(
                ...
                style: GoogleFonts.poppins(
                  color: DesignTokens.textPrimaryDark, fontSize: 20, fontWeight: FontWeight.bold,
                ),
              ),
              const Spacer(),
              Container(
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('Assets', style: GoogleFonts.poppins(color: DesignTokens.textSecondaryDark, fontSize: 11)),
                          Text(
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('Liabilities', style: GoogleFonts.poppins(color: DesignTokens.textSecondaryDark, fontSize: 11)),
                          Text(
  ...
  Widget _buildBreakdown(BuildContext context, AppLocalizations? l10n, Map<String, dynamic> data) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Breakdown', style: GoogleFonts.poppins(color: DesignTokens.textPrimaryDark, fontSize: 16, fontWeight: FontWeight.w600)),
        const SizedBox(height: 12),
        if (assetBreakdown.isNotEmpty) ...[
          Text('Assets', style: GoogleFonts.poppins(color: DesignTokens.textSecondaryDark, fontSize: 12)),
          const SizedBox(height: 8),
          ...assetBreakdown.entries.map((entry) => Container(
            ...
            child: Row(
              ...
              children: [
                Text(_getAssetTypeLabel(entry.key), style: GoogleFonts.poppins(color: DesignTokens.textPrimaryDark, fontSize: 13)),
                Text(
        ...
        if (liabilityBreakdown.isNotEmpty) ...[
          ...
          const SizedBox(height: 8),
          ...liabilityBreakdown.entries.map((entry) => Container(
            margin: const EdgeInsets.only(bottom: 8),
            padding: const EdgeInsets.all(12),
            decoration: BoxDecoration(
              color: DesignTokens.surfaceDark,
            ...
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text(_getLiabilityTypeLabel(entry.key), style: GoogleFonts.poppins(color: DesignTokens.textPrimaryDark, fontSize: 13)),
                Text(
                  CurrencyFormatter.formatRupiah(entry.value.toInt()),
                  style: GoogleFonts.poppins(color: DesignTokens.errorColor, fontSize: 14, fontWeight: FontWeight.w600),
                ),
  ...
  Widget _buildHistory(BuildContext context, AppLocalizations? l10n, List<Map<String, dynamic>> history) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('History Trend', style: GoogleFonts.poppins(color: DesignTokens.textPrimaryDark, fontSize: 16, fontWeight: FontWeight.w600)),
        const SizedBox(height: 12),
        Container(
          padding: const EdgeInsets.all(12),
          decoration: BoxDecoration(
          ...
          child: Column(
            children: history.take(7).map((snapshot) {
              ...
              return Container(
                ...
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text(date, style: GoogleFonts.poppins(color: DesignTokens.textSecondaryDark, fontSize: 12)),
                    Text(
                      CurrencyFormatter.formatRupiah(netWorth.toInt()),
                      style: GoogleFonts.poppins(
                        color: netWorth >= 0 ? DesignTokens.successColor : DesignTokens.errorColor,
