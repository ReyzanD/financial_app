...
class BackupController extends ChangeNotifier {
  ...
  Future<void> loadBackups() async {
    ...
    notifyListeners();
    try {
      _backups = List<File>.from(await _r.listBackups());
    } catch (e) {
  ...
  Future<void> createBackup({bool share = false}) async {
    ...
    notifyListeners();
    try {
      await _r.performBackup(share: share);
      await loadBackups();
    } catch (e) {
      LoggerService.error('Error creating backup', error: e);
      rethrow;
    } finally {
      _isCreating = false;
      notifyListeners();
  ...
  Future<void> deleteBackup(String path) async {
    try {
      await _r.deleteBackup(path);
      await loadBackups();
    } catch (e) {
      LoggerService.error('Error deleting backup', error: e);
      rethrow;
    }
  }
