...
class _ForecastScreenState extends State<ForecastScreen> {
                ...
                children: [
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        Text('Forecast & Prediksi', style: GoogleFonts.poppins(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 20)),
                        Consumer<ForecastController>(builder: (_, ctrl, __) => IconButton(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Gagal Memuat Forecast', style: GoogleFonts.poppins(fontSize: 18, fontWeight: FontWeight.w600, color: Colors.white)),
            const SizedBox(height: 8),
            Text('Butuh data transaksi untuk membuat prediksi', textAlign: TextAlign.center, style: GoogleFonts.poppins(color: DesignTokens.textSecondaryDark)),
            const SizedBox(height: 24),
            ElevatedButton.icon(
              onPressed: ctrl.refresh,
              icon: const Icon(Iconsax.refresh),
              label: const Text('Coba Lagi'),
              style: ElevatedButton.styleFrom(backgroundColor: DesignTokens.primaryColor, foregroundColor: Colors.white),
            ),
          ],
        ),
      ),
  ...
  Widget _buildForecastSummary(ForecastController ctrl) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: DesignTokens.surfaceDark, borderRadius: BorderRadius.circular(16), border: Border.all(color: DesignTokens.borderDark)),
      child: Column(
        ...
        children: [
          Row(children: [
            ...
            Text('Prediksi 30 Hari Kedepan', style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.white)),
          ]),
          const SizedBox(height: 16),
          Text('Rp ${_formatNumber(forecastAmount)}', style: GoogleFonts.poppins(fontSize: 28, fontWeight: FontWeight.w700, color: DesignTokens.primaryColor)),
          const SizedBox(height: 8),
          Row(children: [
            Icon(trendIcon, size: 20, color: trendColor),
            const SizedBox(width: 4),
            Text(
              trend == 'increasing' ? 'Pengeluaran meningkat' : trend == 'decreasing' ? 'Pengeluaran menurun' : 'Pengeluaran stabil',
          ...
          const SizedBox(height: 8),
          Text('Akurasi: ${(confidence * 100).toInt()}%', style: GoogleFonts.poppins(fontSize: 12, color: DesignTokens.textSecondaryDark)),
          const SizedBox(height: 8),
          Container(
            ...
            child: Row(
              ...
              children: [
                Column(children: [
                  ...
                  Text('Rp ${_formatNumber(lower)}', style: GoogleFonts.poppins(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.white)),
                ]),
                Text('95% Confidence', style: GoogleFonts.poppins(fontSize: 11, color: DesignTokens.textTertiaryDark, fontStyle: FontStyle.italic)),
                Column(children: [
                  Text('Maksimum', style: GoogleFonts.poppins(fontSize: 11, color: DesignTokens.textSecondaryDark)),
                  Text('Rp ${_formatNumber(upper)}', style: GoogleFonts.poppins(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.white)),
                ]),
              ],
            ),
          ),
    ...
    return ClipRRect(
      ...
      child: LinearProgressIndicator(
        ...
        valueColor: AlwaysStoppedAnimation<Color>(
          confidence > 0.7 ? DesignTokens.successColor : confidence > 0.5 ? Colors.orange : Colors.red,
        ),
      ),
    );
  }
  ...
  Widget _buildSpendingTrendChart(ForecastController ctrl) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Tren Pengeluaran 3 Bulan', style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.white)),
          const SizedBox(height: 16),
          SizedBox(
            height: 200,
            child: BarChart(
              BarChartData(
                    ...
                    sideTitles: SideTitles(
                      ...
                      getTitlesWidget: (value, meta) {
                        ...
                        return Text(
                          ...
                          style: GoogleFonts.poppins(fontSize: 11, fontWeight: FontWeight.w500, color: DesignTokens.textSecondaryDark),
                        );
                      },
                    ),
                  ),
                ...
                barGroups: List.generate(sortedMonths.length, (i) => BarChartGroupData(
                  x: i,
                  barRods: [
                    BarChartRodData(toY: expenses[i], color: DesignTokens.errorColor, width: 16, borderRadius: const BorderRadius.vertical(top: Radius.circular(4))),
                    BarChartRodData(toY: incomes[i], color: DesignTokens.successColor, width: 16, borderRadius: const BorderRadius.vertical(top: Radius.circular(4))),
                  ],
                )),
              ),
            ),
    ...
    if (critical.isEmpty) {
      return Container(
        ...
        child: Row(children: [
          ...
          Expanded(child: Text('Semua budget dalam kondisi aman', style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w500, color: DesignTokens.successColor))),
        ]),
      );
    }
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const SizedBox(width: 8),
          Text('Peringatan Budget', style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.white)),
        ]),
        const SizedBox(height: 12),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          ...
          Container(
            ...
            child: Text('${usagePercent.toInt()}%', style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.white)),
          ),
        ]),
        const SizedBox(height: 8),
        ClipRRect(
          borderRadius: BorderRadius.circular(4),
          child: LinearProgressIndicator(value: usagePercent / 100, minHeight: 6, backgroundColor: Colors.white.withValues(alpha: 0.1), valueColor: AlwaysStoppedAnimation<Color>(color)),
        ),
        const SizedBox(height: 8),
        Text(
          daysUntil != null ? 'Sisa Rp ${_formatNumber(remaining)} - Habis dalam $daysUntil hari' : 'Sisa Rp ${_formatNumber(remaining)}',
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const SizedBox(width: 8),
          Text('Prediksi per Kategori', style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.white)),
        ]),
        const SizedBox(height: 12),
        ...sorted.take(8).map((entry) => Padding(
          padding: const EdgeInsets.symmetric(vertical: 6),
          child: Row(children: [
            Container(width: 4, height: 24, decoration: BoxDecoration(color: DesignTokens.primaryColor, borderRadius: BorderRadius.circular(2))),
            const SizedBox(width: 12),
            Expanded(child: Text(entry.key, style: GoogleFonts.poppins(fontSize: 14, color: Colors.white))),
            Text('Rp ${_formatNumber(entry.value.toDouble())}', style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w600, color: Colors.white)),
          ]),
        )),
      ]),
    );
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const SizedBox(width: 8),
          Text('Saran Budget Optimal', style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.white)),
        ]),
        const SizedBox(height: 8),
        Text('Berdasarkan rata-rata pengeluaran + buffer 10%', style: GoogleFonts.poppins(fontSize: 12, color: DesignTokens.textSecondaryDark)),
        const SizedBox(height: 12),
        ...sorted.map((entry) => Padding(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(color: DesignTokens.warningColor.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(12), border: Border.all(color: DesignTokens.warningColor.withValues(alpha: 0.3))),
              child: Text('Rp ${_formatNumber(entry.value.toDouble())}', style: GoogleFonts.poppins(fontSize: 13, fontWeight: FontWeight.w600, color: DesignTokens.warningColor)),
            ),
          ]),
        )),
      ]),
  ...
  Widget _buildSpendingInsights(ForecastController ctrl) {
    ...
    if (expenseTrend != null && expenseTrend != 'insufficient_data') {
      final icon = expenseTrend == 'increasing' ? Icons.trending_up : expenseTrend == 'decreasing' ? Icons.trending_down : Icons.trending_flat;
      final color = expenseTrend == 'increasing' ? DesignTokens.errorColor : expenseTrend == 'decreasing' ? DesignTokens.successColor : DesignTokens.textSecondaryDark;
    ...
    if (savingsTrend != null && savingsTrend != 'insufficient_data') {
      final icon = savingsTrend == 'improving' ? Icons.savings : savingsTrend == 'declining' ? Icons.warning : Icons.trending_flat;
      final color = savingsTrend == 'improving' ? DesignTokens.successColor : savingsTrend == 'declining' ? DesignTokens.warningColor : DesignTokens.textSecondaryDark;
    ...
    if (peakDay != null) {
      insights.add(_buildInsightCard(Iconsax.calendar, DesignTokens.infoColor, 'Pengeluaran tertinggi pada hari $peakDay'));
    }
    ...
    if (frequentMerchants.isNotEmpty) {
      ...
      insights.add(_buildInsightCard(Iconsax.shop, DesignTokens.primaryColor, 'Merchant paling sering: ${top['merchant']} (${top['frequency']}x)'));
    }
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const SizedBox(width: 8),
          Text('Wawasan Pengeluaran', style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.white)),
        ]),
        const SizedBox(height: 12),
        ...insights,
      ]),
