...
class _FinancialObligationsScreenState extends State<FinancialObligationsScreen> {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
                    ...
                    suffixIcon: ctrl.searchQuery.isNotEmpty
                        ? IconButton(
                            icon: Container(
                              ...
                              decoration: BoxDecoration(color: DesignTokens.borderDark.withValues(alpha: 0.5), shape: BoxShape.circle),
          ...
          // View Selector
          Consumer<ObligationController>(
            builder: (context, ctrl, _) => ObligationViewTabs(
              selectedView: ctrl.selectedView,
              onViewChanged: (value) => ctrl.setView(value),
            ),
          ),
          // Content based on selected view
          Expanded(child: Consumer<ObligationController>(
      ...
      floatingActionButton: Container(
        decoration: BoxDecoration(
          gradient: const LinearGradient(colors: [DesignTokens.primaryColor, Color(0xFF6B4C93)], begin: Alignment.topLeft, end: Alignment.bottomRight),
          borderRadius: BorderRadius.circular(16),
          boxShadow: [BoxShadow(color: DesignTokens.primaryColor.withValues(alpha: 0.4), blurRadius: 12, offset: const Offset(0, 4))],
        ),
  ...
  Widget _buildSummaryCards(BuildContext context, ObligationController ctrl) {
    ...
    if (ctrl.summaryLoading) {
      return const SizedBox(height: 100, child: Center(child: CircularProgressIndicator(color: DesignTokens.primaryColor)));
    }
              ...
              Expanded(
                child: GestureDetector(
                  ...
                  child: _buildSummaryCard(
                    ...
                    Colors.orange, Iconsax.clock, isCount: true,
                  ),
                ),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: GestureDetector(
                  ...
                  child: _buildSummaryCard(
                    ...
                    Colors.red, Iconsax.warning_2, isCount: true,
                  ),
                ),
              ),
            ],
  ...
  Widget _buildSummaryCard(String title, String amount, Color color, IconData icon, {bool isCount = false}) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [const Color(0xFF1F1F1F), DesignTokens.surfaceDark], begin: Alignment.topLeft, end: Alignment.bottomRight),
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: color.withValues(alpha: 0.25), width: 1.5),
        boxShadow: [BoxShadow(color: color.withValues(alpha: 0.15), blurRadius: 12, spreadRadius: 0, offset: const Offset(0, 4))],
      ),
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  gradient: LinearGradient(colors: [color.withValues(alpha: 0.3), color.withValues(alpha: 0.15)], begin: Alignment.topLeft, end: Alignment.bottomRight),
          ...
          Text(title, style: GoogleFonts.poppins(color: Colors.grey[400], fontSize: 12, fontWeight: FontWeight.w500, letterSpacing: 0.2), maxLines: 2, overflow: TextOverflow.ellipsis),
          const SizedBox(height: 6),
          Text(amount, style: GoogleFonts.poppins(color: Colors.white, fontSize: isCount ? 15 : 18, fontWeight: FontWeight.w700, letterSpacing: 0.3)),
        ],
      ),
    );
  }
