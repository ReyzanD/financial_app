...
class _AddSplitModalState extends State<AddSplitModal> {
  ...
  @override
  void dispose() { _nameCtl.dispose(); _amountCtl.dispose(); _friendCtl.dispose(); super.dispose(); }
    ...
    return Padding(
      ...
      child: SingleChildScrollView(
        child: Form(key: _formKey, child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text('Tambah Split', style: GoogleFonts.poppins(color: DesignTokens.textPrimaryDark, fontSize: 18, fontWeight: FontWeight.w600)),
          const SizedBox(height: 20),
          TextFormField(controller: _nameCtl, style: GoogleFonts.poppins(color: DesignTokens.textPrimaryDark),
          ...
          TextFormField(controller: _amountCtl, keyboardType: TextInputType.number, style: GoogleFonts.poppins(color: DesignTokens.textPrimaryDark),
            decoration: InputDecoration(labelText: l10n?.amount ?? 'Jumlah', labelStyle: GoogleFonts.poppins(color: DesignTokens.textSecondaryDark), filled: true, fillColor: DesignTokens.surfaceDark,
              border: OutlineInputBorder(borderRadius: BorderRadius.circular(DesignTokens.radiusMedium), borderSide: BorderSide(color: DesignTokens.borderDark))),
            validator: (v) => (v == null || v.isEmpty) ? 'Jumlah tidak boleh kosong' : null),
          const SizedBox(height: 16),
          TextFormField(controller: _friendCtl, style: GoogleFonts.poppins(color: DesignTokens.textPrimaryDark),
            decoration: InputDecoration(labelText: 'Nama Teman', labelStyle: GoogleFonts.poppins(color: DesignTokens.textSecondaryDark), filled: true, fillColor: DesignTokens.surfaceDark,
              border: OutlineInputBorder(borderRadius: BorderRadius.circular(DesignTokens.radiusMedium), borderSide: BorderSide(color: DesignTokens.borderDark))),
  ...
  Future<void> _save() async {
    ...
    try {
      await getIt<ExpenseSplitService>().createSplit(SplitModel(
        id: 'split_${DateTime.now().millisecondsSinceEpoch}',
        transactionId: '',
        participantName: _nameCtl.text, amount: double.tryParse(_amountCtl.text) ?? 0.0,
        createdAt: DateTime.now(), isSettled: false,
      ));
      if (!mounted) return; Navigator.pop(context); widget.onSplitAdded();
