...
class _SplitsScreenState extends State<SplitsScreen> {
          ...
          Column(
            ...
            children: [
              Text(
                ...
                style: GoogleFonts.poppins(
                  color: split.isSettled ? DesignTokens.successColor : DesignTokens.warningColor,
                  fontSize: 14,
                  fontWeight: FontWeight.w600,
                ),
              ),
  ...
  Widget _buildError(BuildContext context, AppLocalizations? l10n, SplitController c) => Center(
    child: Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
  ...
  Future<void> _delete(BuildContext context, SplitModel split) async {
    ...
    if (confirmed == true) {
      try {
        ...
        if (!mounted) return;
        ErrorHandlerService.showSuccessSnackbar(context, l10n?.split_deleted_successfully ?? 'Split berhasil dihapus');
      } catch (e) {
        if (!mounted) return;
